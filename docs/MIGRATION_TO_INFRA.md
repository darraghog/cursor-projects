...
Transcription-demo currently runs against **original infrastructure** (not created by the [transcription-demo-infra](../transcription-demo-infra) project). This doc tracks migrating to infra defined and deployed by transcription-demo-infra (CDK: S3 bucket, Lambda, S3 trigger).
...
2. **Point transcription-demo scripts to new infra** – Use bucket name from CDK output (`TranscriptionDemoLambda` → BucketName). Set `--bucket` or `TRANSCRIPTION_DEMO_BUCKET` in scripts/env.
...
- All infrastructure will be created and updated via `transcription-demo-infra` (CDK).
