...
|-----------|-------------------|----------|
| **Lambda: `extract_transcript_from_textract`** | JSON → speaker-labeled text | Pure function: no mocks. Feed Transcribe-style JSON, assert output string. |
...
- **`tests/test_extract_transcript.py`** – `extract_transcript_from_textract`: empty items, with/without speaker labels, punctuation handling.
